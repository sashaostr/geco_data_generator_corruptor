...
class CorruptNothing(CorruptValue):
  """A corruptor method which corrupts nothing
  """
...
class CorruptDataSet:
  ...
  def __init__(self, **kwargs):
    ...
    self.nothing_corruptor = CorruptNothing()
    ...
    for (attr_name, attr_mod_data_list) in self.attr_mod_data_dict.items():
      ...
      total_prob = sum(map(lambda c: c[0], attr_mod_data_list))
      if total_prob < 1:
        attr_mod_data_list = [(1-total_prob, self.nothing_corruptor)] + attr_mod_data_list
        self.attr_mod_data_dict[attr_name] = attr_mod_data_list
